...
* 📥 **Responsive to new file uploads:** If content contains a file link before the file exists, the presave hooks store that URL. Upload the file and run a scan (or let cron do it) to register usage **only if the placeholder link remained in `filelink_usage_matches`**. If a scan removed the unmatched link before the file was added, usage will not be created until the content is saved again.
...
* 🪝 **Immediate updates via hooks:** Presave hooks for nodes, custom blocks, taxonomy terms, comments, and paragraphs call the manager’s `manageUsage()` method. This stores every detected file URL in `filelink_usage_matches` and adds usage immediately if the file already exists. When the scanner runs later, any links without files are purged. Delete hooks remove stored matches and usage. File entity inserts also check for saved links referencing the new file and add usage only when that link entry still exists.
...
* **FileLinkUsageManager** (`filelink_usage.manager` service): This service coordinates higher-level operations and integration with Drupal’s lifecycle:
  ...
  * The manager also responds to **file insertions**. When a new file is added to the system, the manager’s `addUsageForFile()` is invoked via hook_entity_insert. This method checks if the file’s URI matches any link that had been previously recorded in `filelink_usage_matches` (for which no file was found at the time). If that record still exists—meaning a scan has not removed it—it uses the File Usage service to add a usage for the corresponding entity. This allows the module to “retroactively” tie the file to content that referenced it.
